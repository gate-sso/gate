...
def ntlm_challenge_response word, challenge
    ...
    return hexlify(response[0]) + hexlify(response[1]) + hexlify(response[2])
...
def main
    if ARGV.length == 4
        ...
        challenege     	= unhexlify ARGV[1].strip
        response        = unhexlify ARGV[2].strip
	    vpn_server_ip	= ARGV[3].strip # this is not handled in this script
        otp = get_otp username
        if ntlm_challenge_response(otp, challenege) == hexlify(response)
            puts "NT_KEY: " + get_nt_key(otp).upcase
        else
            abort("NT_STATUS_UNSUCCESSFUL: Failure (0xC0000001)")
        end

    else
        abort("Usage: #{$PROGRAM_NAME} <username> <challenge> <response>")
    end
end
